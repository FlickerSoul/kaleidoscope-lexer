...
func branch(_ children: [Character: NodeId], _ miss: NodeId? = nil, inverted _: Bool = false) -> Node {
    .branch(.init(
        branches: Dictionary(uniqueKeysWithValues: children.map { ($0.scalarByte.scalarByteRange, $1) }),
        miss: miss,
    ))
}

func branch(_ children: [Node.BranchHit: NodeId], _ miss: NodeId? = nil, inverted _: Bool = false) -> Node {
    .branch(.init(branches: children, miss: miss))
}

func seq(_ seq: String, _ then: NodeId, _ miss: Node.SeqMiss? = nil) -> Node {
    .seq(.init(seq: seq.map { $0.into() }, then: then, miss: miss))
}
    ...
    func buildGraph() throws {
        let tests: [(regexs: [String], nodes: [Node?], root: NodeId)] = [
            ...
            (
                regexs: ["ab", "[a-b]+?"],
                nodes: [
                    branch([range("a", "b"): 3], 5),
                    seq("b", 0, .anytime(3)),
                    branch(["a": 1, "b": 3]),
                    branch([range("a", "b"): 3], 4),
                    leaf(1),
                    leaf(0),
                ],
                root: 2,
            ...
            (
                ...
                nodes: [
                    seq("b", 3),
                    branch([0 ... 96: 2, 98 ... HIR.ScalarByte.max: 2, 97 ... 97: 0]),
                    leaf(1),
                    leaf(0),
                ],
                root: 1,
            ...
            (
                regexs: ["ab", "[^bc]+?"],
                nodes: [
                    branch([0 ... 97: 2, 98 ... 98: 4, 100 ... HIR.ScalarByte.max: 2], 3),
                    branch([97 ... 97: 0, 100 ... HIR.ScalarByte.max: 2, 0 ... 96: 2]),
                    branch([0 ... 97: 2, 100 ... HIR.ScalarByte.max: 2], 3),
                    leaf(1),
                    leaf(0),
                ],
                root: 1,
